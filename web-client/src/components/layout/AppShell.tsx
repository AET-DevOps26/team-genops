    ...
    <NavLink
      ...
      className={({ isActive }) =>
        `flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm transition ${
          isActive ? 'bg-raised-2 text-fg' : 'text-dim hover:bg-raised-2/60 hover:text-fg'
        }`
      }
    >
...
const ICONS = {
  ...
  applications: (
    <svg width="15" height="15" viewBox="0 0 16 16" fill="none" aria-hidden>
      <rect x="1.5" y="3" width="13" height="10.5" rx="1.5" stroke="currentColor" strokeWidth="1.3" />
  ...
  profile: (
    <svg width="15" height="15" viewBox="0 0 16 16" fill="none" aria-hidden>
      ...
      <path d="M2.5 14c.8-2.6 3-4 5.5-4s4.7 1.4 5.5 4" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" />
    </svg>
  ),
  assistant: (
    <svg width="15" height="15" viewBox="0 0 16 16" fill="none" aria-hidden>
      <path d="M2 3.5A1.5 1.5 0 0 1 3.5 2h9A1.5 1.5 0 0 1 14 3.5v6a1.5 1.5 0 0 1-1.5 1.5H8l-3.5 3v-3h-1A1.5 1.5 0 0 1 2 9.5v-6Z" stroke="currentColor" strokeWidth="1.3" strokeLinejoin="round" />
    </svg>
  ),
}
...
export function AppShell() {
  ...
  const needsOnboarding = profileError != null && 'status' in profileError && profileError.status === 404
