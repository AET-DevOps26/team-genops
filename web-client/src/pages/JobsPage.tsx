...
const SAMPLE_JOBS = [
  ...
  },
  {
    ...
    description: 'Support the core platform team building process-mining infrastructure.',
  },
  {
    company: 'Zalando',
  ...
  },
  {
    ...
    description: 'Ship features end-to-end across the HR platform with a senior mentor.',
  },
  {
    company: 'Stripe',
    ...
    type: 'Full-time',
    tags: ['Ruby', 'Java', 'APIs'],
    description: 'Design APIs used by millions of businesses to move money safely.',
  },
  {
    company: 'SAP',
    ...
    description: 'Operate the delivery platform for one of the largest B2B clouds.',
  },
]
    ...
    <div className="h-full overflow-y-auto">
      <div className="mx-auto max-w-5xl px-8 py-8">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {SAMPLE_JOBS.map((job) => (
            <Card key={`${job.company}-${job.role}`} className="flex flex-col p-5">
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                {job.tags.map((tag) => (
                  <span key={tag} className="rounded-full bg-raised-2 px-2.5 py-1 text-xs text-dim">
                    {tag}
                  </span>
                ))}
              </div>
              <div className="mt-4 flex items-center justify-between border-t border-line pt-4">
                ...
                <Button className="px-4 py-2 text-sm" onClick={() => apply(job)}>
                  Apply with JobReady
                </Button>
              </div>
            </Card>
