...
export default function ChatPage() {
  ...
  useEffect(() => {
    if (sessionsLoading || !sessionsData || activeSessionId || bootstrapped.current) return
            ...
            <div className="flex flex-col items-center justify-center h-full gap-4 text-center anim-rise">
              <div className="grid h-12 w-12 place-items-center rounded-full bg-offer/15 text-offer">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" aria-hidden>
                  <path d="M12 3C6.48 3 2 6.92 2 11.75c0 2.6 1.28 4.94 3.32 6.6L4 21l3.5-1.75A11.3 11.3 0 0 0 12 20.5c5.52 0 10-3.92 10-8.75S17.52 3 12 3Z" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round"/>
                </svg>
              </div>
              <div className="space-y-1">
                <p className="text-fg text-sm font-medium">How can I help you today?</p>
                <p className="text-faint text-xs">Try a command or just ask a question</p>
              </div>
              <div className="flex flex-wrap gap-2 justify-center">
                {['/cover_letter', '/resume_tailor', '/fit_analysis'].map((cmd) => (
                  <span
                    key={cmd}
