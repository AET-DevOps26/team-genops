...
function ApplicationForm({ initial, editing, onClose }: {
  ...
  const [extract, { isLoading: extracting, error: extractError, reset: resetExtract }] =
    useExtractJobPostingMutation()
        ...
        <form onSubmit={onSubmit} className="space-y-4 px-1 py-5">
          ...
          <div className="rounded-lg border border-line bg-raised-2/40 p-3.5">
            ...
            <p className="mt-1.5 text-xs text-faint">
              Paste the posting link and the assistant fills in company, role, and description.
            </p>
            {extractError != null && (
              <div className="mt-2 space-y-1">
                <ErrorBanner error={extractError as NormalizedError} />
                <p className="text-xs text-faint">You can still fill in the details manually below.</p>
              </div>
            )}
          </div>
          <Field id="company" label={<>Company<Req /></>} required value={form.company} onChange={set('company')} />
          <Field id="job_title" label={<>Role<Req /></>} required value={form.job_title} onChange={set('job_title')} />
          <div>
          ...
          <div>
            ...
            {!editing && (
              <p className="mt-1.5 text-xs text-faint">
                Starts in Draft — you&apos;re preparing this application. Pick another stage if
