...
import { Button, Card, ErrorBanner, Field, StatusDot, Tag } from '~/components/ui'
import type { NormalizedError } from '~/api/errors'
import type { ApplicationStage, GeneratedDocumentType, JobApplication } from '~/api/schemas'
import {
...
function ApplicationForm({ initial, editing, onClose }: {
  initial: FormState
  editing: JobApplication | null
  onClose: () => void
}) {
  const [form, setForm] = useState<FormState>(initial)
  const [create, { isLoading: creating, error: createError }] = useCreateApplicationMutation()
  const [update, { isLoading: updating, error: updateError }] = useUpdateApplicationMutation()
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      if (editing) {
        await update({ id: editing.id, body: { ...body, stage: form.stage } }).unwrap()
      } else {
  ...
  return (
    <div className="fixed inset-0 z-40 flex justify-end bg-black/50" onClick={onClose}>
      <Card
        ...
        header={
          <>
            <Tag className="text-dim">{editing ? 'Edit application' : 'New application'}</Tag>
            <button onClick={onClose} className="tag text-faint transition hover:text-fg">
              Close
            </button>
          </>
        }
      >
        <form onSubmit={onSubmit} className="space-y-4 px-1 py-5">
          <Field id="company" label="Company" required value={form.company} onChange={set('company')} />
          <Field id="job_title" label="Role" required value={form.job_title} onChange={set('job_title')} />
          <div>
            <label htmlFor="job_description" className="tag mb-1.5 block text-dim">
              Job description
            </label>
            <textarea
              id="job_description"
          ...
          <Field id="job_url" label="Job URL" type="url" value={form.job_url} onChange={set('job_url')} placeholder="https://…" />
          <div>
            <label htmlFor="notes" className="tag mb-1.5 block text-dim">Notes</label>
            <textarea
              id="notes"
              rows={2}
              value={form.notes}
          ...
          {editing && (
            <div>
              <label htmlFor="stage" className="tag mb-1.5 block text-dim">Stage</label>
              <select
                id="stage"
                value={form.stage}
                onChange={set('stage')}
          ...
          <ErrorBanner error={(createError ?? updateError) as NormalizedError | undefined} />
...
function DocumentsTab({ application, type }: { application: JobApplication; type: GeneratedDocumentType }) {
  const navigate = useNavigate()
  const { data, isLoading } = useGetDocumentsQuery({ applicationId: application.id })
  ...
  return (
    <div className="space-y-4">
      {docs.map((doc) => (
        <div key={doc.id} className="rounded-lg border border-line bg-raised-2/40 p-4">
          <div className="mb-2 flex items-center justify-between">
            <Tag className="text-faint">{new Date(doc.created_at).toLocaleString()}</Tag>
...
function TimelineTab({ application }: { application: JobApplication }) {
  ...
  if (events.length === 0) {
    return (
      <p className="text-sm text-dim">
        No timeline events yet. Stage changes and detected emails will show up here with their
        exact dates.
      </p>
    )
  }
  ...
  return (
    <ol className="space-y-4">
      {events.map((event) => (
        <li key={event.id} className="rounded-lg border border-line bg-raised-2/40 p-4">
          <div className="mb-1 flex items-center justify-between">
            <span className="text-sm font-medium">{event.title}</span>
            <Tag className="text-faint">{new Date(event.occurred_at).toLocaleString()}</Tag>
          </div>
          {event.description && <p className="text-sm text-dim">{event.description}</p>}
          <div className="mt-2 flex items-center gap-2">
            <Tag className="text-faint">{event.event_type.replace(/_/g, ' ')}</Tag>
            <Tag className="text-faint">{event.source === 'email' ? 'from email' : 'manual'}</Tag>
            {event.stage_from && event.stage_to && (
              <Tag className="text-dim">
                {stageMeta(event.stage_from).label} → {stageMeta(event.stage_to).label}
              </Tag>
            )}
          </div>
        </li>
...
function ActionItemsTab({ application }: { application: JobApplication }) {
  ...
  if (isLoading) return <p className="text-sm text-dim">Loading action items…</p>
  if (items.length === 0) {
    return (
      <p className="text-sm text-dim">
        No action items right now. Suggestions like following up or preparing for an interview
        appear here when they're derived from your emails.
      </p>
    )
  }
  ...
  return (
    <ul className="space-y-4">
      {items.map((item) => (
        <li key={item.id} className="rounded-lg border border-line bg-raised-2/40 p-4">
          <div className="mb-1 flex items-center justify-between">
            <Tag className="text-faint">{new Date(item.created_at).toLocaleString()}</Tag>
            <button
              className="tag text-faint transition hover:text-interview"
              onClick={() =>
                deleteRecommendation({ applicationId: application.id, recommendationId: item.id })
              }
            >
              Dismiss
            </button>
...
type DrawerTab = 'overview' | 'timeline' | 'actions' | 'cover_letter' | 'resume'

function DetailDrawer({ application, onEdit, onClose }: {
    ...
    <div className="fixed inset-0 z-40 flex justify-end bg-black/50" onClick={onClose}>
        ...
        header={
          <>
            <span className="flex items-center gap-2.5">
              ...
              <span>
                <span className="block text-sm font-medium">{application.company}</span>
                <span className="block text-xs text-dim">{application.job_title}</span>
              </span>
            </span>
      ...
      >
        ...
        <div className="p-6">
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="mt-1">{new Date(application.applied_at).toLocaleDateString()}</p>
                </div>
                <div>
                  <Tag className="text-faint">Updated</Tag>
                  <p className="mt-1">{new Date(application.updated_at).toLocaleDateString()}</p>
                </div>
              </div>
              {application.job_url && (
                <div>
              ...
              {application.notes && (
                <div>
                  ...
                  <p className="mt-1 whitespace-pre-wrap text-sm text-dim">{application.notes}</p>
                </div>
              )}
              <Button variant="ghost" className="px-4 py-2 text-sm" onClick={onEdit}>
                Edit application
              </Button>
            </div>
          )}
          ...
          {tab === 'cover_letter' && <DocumentsTab application={application} type="cover_letter" />}
          {tab === 'resume' && <DocumentsTab application={application} type="resume" />}
        </div>
      </Card>
    </div>
  )
      ...
      <div className="mx-auto max-w-5xl px-8 py-8">
        <div className="mb-6 flex items-end justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-dim">Every application you&apos;re tracking</p>
          </div>
          <Button
        ...
        <div className="mb-4 flex flex-wrap gap-2">
          {[{ value: 'all' as const, label: 'All', color: 'var(--color-dim)' }, ...STAGES].map((s) => (
            <button
              key={s.value}
            ...
            >
              {s.value !== 'all' && <StatusDot color={s.color} className="h-2 w-2" />}
              {s.label}
            </button>
          ))}
        </div>
        ...
        <Card>
          ...
          {!isLoading && filtered.length === 0 && (
            <p className="p-8 text-center text-sm text-dim">
              {filter === 'all' ? 'Nothing tracked yet — add your first application.' : 'No applications in this stage yet.'}
            </p>
          )}
          {filtered.length > 0 && (
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-line">
                  <th className="tag px-5 py-3 font-normal text-faint">Company</th>
                  <th className="tag px-5 py-3 font-normal text-faint">Role</th>
                  <th className="tag px-5 py-3 font-normal text-faint">Stage</th>
                  <th className="tag px-5 py-3 font-normal text-faint">Applied</th>
                      ...
                      <td className="px-5 py-3.5 text-right">
                        <span className="flex justify-end gap-3">
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation()
                              if (window.confirm(`Delete the ${a.company} application?`)) {
