...
function deriveStats(items: JobApplication[]) {
  const byStage = Object.fromEntries(STAGES.map((s) => [s.value, 0])) as Record<string, number>
  ...
  const responseRate = items.length ? Math.round((responded / items.length) * 100) : 0
    ...
    <div className="h-full overflow-y-auto">
      <div className="mx-auto max-w-5xl px-8 py-8">
        <div className="mb-7 flex items-end justify-between">
          ...
          <Button className="px-4 py-2 text-sm" onClick={() => navigate('/applications?create=1')}>
            New application
          </Button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          />
          <StatTile label="Interviews" value={String(byStage.interview)} detail="in the pipeline" />
          <StatTile label="Offers" value={String(byStage.offer)} detail="on the table" />
          <StatTile
            label="Response rate"
            value={`${responseRate}%`}
        ...
        <div className="mt-8 mb-3 flex items-baseline justify-between">
          ...
          <Link to="/applications" className="tag text-faint transition hover:text-fg">
            View all
          </Link>
        </div>
            ...
            return (
              <Card key={stage.value} className="p-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono text-sm text-dim">{inStage.length}</span>
                </div>
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-[1fr_320px]">
          <Card className="p-5">
            <h2 className="mb-4 font-display text-lg font-semibold tracking-tight">Recent applications</h2>
            {isLoading && <p className="text-sm text-dim">Loading…</p>}
            {!isLoading && recent.length === 0 && (
              <p className="text-sm text-dim">
                Nothing tracked yet.{' '}
                <Link to="/applications?create=1" className="text-offer hover:underline">
                ...
                return (
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="block truncate text-sm">{a.company}</span>
                      <span className="block truncate text-xs text-dim">{a.job_title}</span>
                    </span>
          ...
          <Card className="p-5">
            ...
            <p className="mt-2 text-sm text-dim">
              Draft a tailored cover letter, tune your resume, or check your fit for a role.
            </p>
            <Button className="mt-4 w-full px-4 py-2.5 text-sm" onClick={() => navigate('/chat')}>
              Open AI Assistant
            </Button>
          </Card>
        </div>
