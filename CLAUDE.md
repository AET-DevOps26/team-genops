...
- **Dashboards are code** — Grafana dashboard JSON files live in `infra/helm/monitoring/files/dashboards/` (single source for local compose AND prod).
...
│   ├── prometheus/      # Scrape config + alerts/rules.yml (4 alert rules)
│   └── grafana/         # Provisioning + dashboards/*.json (dashboards are code)
├── docs/
