...
**Observability stack:** Prometheus (metrics: request count, latency, error rate) + Grafana (dashboards committed as `.json` files) with alert rules for service health. **Langfuse** provides LLM observability for `genai` — per-call traces, token usage, and cost, attributed by `user_id`/`session_id`. It is self-hosted locally (opt-in docker-compose under `monitoring/langfuse/`) and uses **Langfuse Cloud** for deployed environments; see `monitoring/README.md`.
...
├── monitoring/          # Opt-in local stack (`monitoring` compose profile)
│   ├── docker-compose.yml   # Aggregator — `include:`s each tool below
│   ├── langfuse/        # Self-hosted Langfuse + model-pricing seeder
│   ├── prometheus.yml   # Scrape config (planned)
│   ├── alerts/          # Alert rule YAML files (planned)
│   └── grafana/
│       └── dashboards/  # Exported Grafana dashboard JSON files (planned)
├── docs/
...
> **Submission readiness:** `docs/submission-readiness-plan.md` is the **living roadmap** for
> final submission — a gap analysis against the course requirements plus a progress tracker and
> 7 workstreams (each expanded into its own detailed plan). Check it before picking up work.
...
- `application` — ✅ functional (Spring Boot): applications CRUD with stage filter + pagination + optional company website/LinkedIn fields, per-stage `/summary` endpoint, stored per-application `Recommendation` items, append-only `ApplicationEvent` timeline (`GET /api/v1/applications/{id}/events`; manual stage changes recorded too), static-token `/internal/v1` API for the email pipeline incl. auto-create (see `services/application/README.md` — the one sanctioned exception to "user_id from JWT only"), unit + web-layer security tests, verify-only JWT (owner from `sub`).
- `email` — ✅ functional (Spring Boot): Gmail OAuth2 connect/disconnect (signed single-use state, tokens AES-GCM-encrypted at rest), `@Scheduled` background poller (full bodies), verify-only JWT (owner from `sub`), Flyway schema, unit + web-layer security tests. Application-detection pipeline (see `services/email/README.md`): keyword pre-filter → genai LLM classification (subject/body-based, sender-agnostic) → confident matches update the application (stage + timeline event + action items) or auto-create one for an untracked company (≥0.8 confidence); blank `INTERNAL_SERVICE_TOKEN` disables it (fail-closed). Single-instance assumptions (in-process nonce store + poller) — needs Redis for multi-replica.
