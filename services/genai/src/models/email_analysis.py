...
class TimelineEvent(BaseModel):
    ...
    event_type: str = Field(
        description="One of: stage_change, email_received, interview_scheduled, "
    ...
    description: str = Field(
        description="1-3 sentence factual summary of what the email says happened"
    )
...
class EmailAnalysisResult(BaseModel):
    """Verdict on one email: is it about one of the user's job applications, and if so what changed."""

    relevant: bool = Field(
        description="True only if this email is about one of the candidate job applications. "
        "Newsletters, job-board digests, promotions and unrelated mail are not relevant."
    )
    ...
    confidence: float = Field(
        ge=0.0, le=1.0, description="Confidence in the match and interpretation, 0.0-1.0."
    )
    suggested_stage: str | None = Field(
        default=None,
        description="New stage if the email clearly implies a FORWARD transition "
        "(applied → follow_up → interview → offer → closed), else null.",
    )
    event: TimelineEvent | None = Field(
        default=None, description="Timeline entry for the email; null if not relevant."
    )
    action_items: list[ActionItem] = Field(
        default_factory=list,
        description="Concrete next steps for the user (follow up, send requested documents, "
        "prepare for a scheduled interview). Empty if none.",
    )
