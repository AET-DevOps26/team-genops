...
public class AuthController implements AuthApi {
    ...
    private HttpHeaders sessionCookies(IssuedSession session) {
        ...
        headers.add(HttpHeaders.SET_COOKIE, cookie(cookieProperties.getAccessName(),
            session.accessToken(), cookieProperties.getAccessPath(), session.accessMaxAge()).toString());
        headers.add(HttpHeaders.SET_COOKIE, cookie(cookieProperties.getRefreshName(),
            session.refreshToken(), cookieProperties.getRefreshPath(), session.refreshMaxAge()).toString());
        return headers;
    }
    ...
    private HttpHeaders clearedCookies() {
        HttpHeaders headers = new HttpHeaders();
        headers.add(HttpHeaders.SET_COOKIE, cookie(cookieProperties.getAccessName(),
            "", cookieProperties.getAccessPath(), Duration.ZERO).toString());
        headers.add(HttpHeaders.SET_COOKIE, cookie(cookieProperties.getRefreshName(),
            "", cookieProperties.getRefreshPath(), Duration.ZERO).toString());
        return headers;
    }
