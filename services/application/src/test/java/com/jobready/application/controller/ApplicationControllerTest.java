...
class ApplicationControllerTest {
    ...
    void create_returns201WithBody() throws Exception {
        when(applicationService.create(eq(userId), any())).thenReturn(
            new JobApplication()
                .id(UUID.randomUUID())
                .company("Acme")
                .jobTitle("Engineer")
    ...
    void foreignApplication_surfacesAs404() throws Exception {
        ...
        mvc.perform(get("/api/v1/applications/{id}", foreignId)
                .with(jwt().jwt(j -> j.subject(userId.toString()))))
    ...
    void recommendationsOnForeignApplication_surfaceAs404() throws Exception {
        UUID foreignId = UUID.randomUUID();
        when(applicationService.listRecommendations(userId, foreignId))
    ...
    void deleteRecommendation_returns204() throws Exception {
        mvc.perform(delete("/api/v1/applications/{id}/recommendations/{rid}",
    ...
    void summary_isReachableAndNotShadowedByIdRoute() throws Exception {
        when(applicationService.summary(userId)).thenReturn(
            new com.jobready.application.generated.modelDto.ApplicationSummary()
                .applied(1L).followUp(0L).interview(0L).offer(0L).closed(0L).total(1L));

        mvc.perform(get("/api/v1/applications/summary")
                .with(jwt().jwt(j -> j.subject(userId.toString()))))
            .andExpect(status().isOk())
